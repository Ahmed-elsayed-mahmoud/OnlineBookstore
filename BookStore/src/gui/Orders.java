...
public class Orders extends JFrame {
	...
	private Orders frame ;
	private JTable table;
	
	/*
	 * Create the frame.
	 */
	public Orders(final java.sql.Statement statement ,final Connection connection , final User usr) {
		frame = this ;
		...
		confirmbtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				else
				{
					...
					try {
						((Manager)usr).confirmOrder(isbn) ;
					} catch (SQLException e) {
						// TODO Auto-generated catch block
						JOptionPane.showMessageDialog(frame, "cann't confirm orders");
						e.printStackTrace();
					}
					
					readOrders(usr, table);
				}
				
			}
		});
		confirmbtn.setFont(new Font("Tahoma", Font.PLAIN, 14));
		confirmbtn.setIcon(new ImageIcon("pics\\confirm.png"));
		...
		backbtn.setIcon(new ImageIcon("pics\\back-mb-icon.png"));
	...
	private void readOrders(User usr, JTable table2) {
		// TODO Auto-generated method stub
		...
		try {
			ResultSet resultSet = ((Manager) usr).getOrders() ;
